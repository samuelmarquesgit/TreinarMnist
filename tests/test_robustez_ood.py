...
def test_relatorio_overconfidence_lanca_typeerror():
    ...
    modelo_invalido = ModeloInvalidoSemNada()

    # O Duck Typing deve falhar se o modelo não tiver nenhum método de predição
    with pytest.raises(TypeError, match="O modelo fornecido não possui métodos de predição suportados"):
        obter_probabilidades(modelo_invalido, np.array([[1]]))
