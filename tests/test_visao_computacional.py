...
def test_visao_computacional_todas_excecoes_e_branches(tmp_path):
    ...
    with (
        patch("src.visao_computacional.cv2.imread", return_value=None),
        pytest.raises(ValueError, match="decodificar o arquivo"),
    ):
        _carregar_imagem(str(fake_img))
    ...
    canvas = aplicar_padding_centralizado(
        np.zeros((10, 10), dtype=np.uint8), usar_centro_massa=False
    )
    ...
    with patch("src.visao_computacional.cv2.moments") as mock_mom:
        ...
        canvas2 = aplicar_padding_centralizado(
            np.zeros((10, 10), dtype=np.uint8), usar_centro_massa=True
        )
    ...
    with patch(
        "src.visao_computacional.redimensionar_com_proporcao",
        side_effect=ValueError("Mock erro"),
    ):
    ...
    with patch(
        "src.visao_computacional.aplicar_padding_centralizado",
        side_effect=ValueError("Mock erro"),
    ):
        res = preprocessar_imagem_mnist(img_valida)
        assert np.all(res == 0)
