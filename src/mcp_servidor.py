...
def treinar_modelo_mnist(nome_modelo: str) -> str:
    ...
    try:
        ...
        logger.error(f"[MCP] Erro ao treinar '{nome_modelo}': {e}")
...
def consultar_rag_mnist(pergunta: str) -> List[str]:
    ...
    try:
        ...
        logger.error(f"[MCP] Erro na consulta RAG: {e}")
        return [f"❌ Erro na consulta RAG: {str(e)}"]


if __name__ == "__main__":
    # Inicia o servidor MCP via Stdio (comunicação nativa com agentes)
