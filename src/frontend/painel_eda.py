...
def renderizar(fachada) -> None:
    ...
    with col_graf:
        if PLOTLY_OK:
            fig = px.bar(
                ...
                text="Quantidade",
                labels={"Quantidade": "Nº de amostras"},
            )
    ...
    with col_heat:
        ...
        if PLOTLY_OK:
            ...
            fig_h.update_layout(
                **_TEMA, margin={"t": 5, "b": 5}, height=220, coloraxis_showscale=True
            )
    ...
    st.caption(
        "Projeção matemática das 784 dimensões em 2D para visualização topológica das classes."
    )
