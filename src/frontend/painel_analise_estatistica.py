...
def _obter_dados(fachada, modo: str, particao: str, digito: int | None) -> np.ndarray:
...
def _card_metricas(stats_dict: dict) -> None:
    ...
    for i, (chave, label) in enumerate(mapa):
        ...
        cols[i % 4].markdown(kpi_tile(f"{val:.4f}", label), unsafe_allow_html=True)
...
def renderizar(fachada) -> None:
    ...
    with col1:
        modo = st.radio(
            "Tipo de dado", ["Brutos [0–255]", "Tratados [0–1] MinMax"], horizontal=True
        )
    with col2:
    ...
    if PLOTLY_OK:
        ...
        with abas[1]:
            ...
            for cls in range(10):
                ...
                amostra_cls = (
                    medias_cls
                    if len(medias_cls) <= 2000
                    else np.random.choice(medias_cls, 2000, replace=False)
                )
                for v in amostra_cls:
                    amostras_box.append({"Dígito": str(cls), "Intensidade Média": v})
        ...
        with abas[3]:
            ...
            if digito_filtro is not None:
                ...
                media_espacial = X_base[mask].mean(axis=0).reshape(28, 28) * fator
            ...
            fig_heat = px.imshow(
                ...
                zmin=0,
                zmax=(255 if "Brutos" in modo else 1),
                labels={"color": "Intensidade"},
            )
            fig_heat.update_layout(
                **_TEMA, height=380, margin={"t": 30, "b": 10}, title=titulo_heat
            )
    ...
    with abas_testes[0]:
        ...
        amostra_norm = (
            amostra_norm
            if len(amostra_norm) <= 5000
            else np.random.choice(amostra_norm, 5000, replace=False)
        )
    ...
    with abas_testes[1]:
        ...
        if digito_a == digito_b:
            ...
            concl = (
                "✅ Médias **não diferem** significativamente (p > 0.05)"
                if p_t > 0.05
                else f"❌ Médias diferem significativamente — dígito {digito_a} ≠ dígito {digito_b} (p ≤ 0.05)"
            )
            st.markdown(concl)
    ...
    with abas_testes[2]:
        ...
        concl_anova = (
            "✅ Não há diferença significativa entre as classes (p > 0.05)"
            if p_f > 0.05
            else "❌ Há diferença significativa de brilho entre as 10 classes de dígitos (p ≤ 0.05)"
        )
    ...
    with abas_testes[3]:
        ...
        concl_chi = (
            "✅ Distribuição **uniforme** (balanceada) — p > 0.05"
            if p_chi > 0.05
            else "❌ Distribuição **não uniforme** entre classes — p ≤ 0.05"
        )
