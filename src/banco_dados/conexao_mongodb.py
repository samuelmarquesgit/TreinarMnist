...
class ConexaoMongoDB:
    ...
    def __init__(self, uri: str = None) -> None:
        ...
        if not self.usar_local:
            try:
                ...
                # Tenta forcar erro caso URI seja invalida e servidor inacessivel
                self.client.server_info()
                logger.info("Conexao com MongoDB Cloud inicializada.")
            except Exception as e:
                logger.warning(f"Falha ao conectar no MongoDB Cloud. Fazendo fallback local: {str(e)}")
                self.usar_local = True
        else:
            logger.info("MONGO_URI nao definida. Utilizando armazenamento local Json.")
    ...
    def salvar_artefato(self, nome: str, dados: Dict[str, Any]) -> None:
        ...
        if self.usar_local:
            ...
            logger.info(f"Artefato {nome} salvo em formato JSON local: {caminho_arquivo}")
        else:
